...
foreach ($pr in $candidates) {
    ...
    $ciBlockingMerge = $baConclusion -eq "FAILURE" -and $approvalCount -gt 0 -and $unresolvedThreads -eq 0 -and -not $hasNeedsAuthorAction
    $valueRaw = 0.0
    ...
    if ($ciBlockingMerge) { $valueRaw += 1.5 }  # CI blocking merge — approved but can't land
    ...
    if ($ciBlockingMerge) { $valueWhy += "CI blocking merge — approved but can't land (+1.5)" }
    ...
    if ($ciBlockingMerge) { $allContribs += [PSCustomObject]@{ key = "ci blocking"; text = "CI blocking merge"; pts = 1.5 } }
